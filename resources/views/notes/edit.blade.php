        ...
        <form id="noteForm" action="{{ route('notes.update', $note->id) }}" method="POST"
            ...
            <div class="flex items-center justify-between shrink-0">
                ...
                <select
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" class="text-gray-900" {{ old('category_id', $note->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex-1 overflow-y-auto my-4">
                <div id="richTextEditor" contenteditable="true" role="textbox" aria-multiline="true"
                    class="w-full h-full min-h-37.5 border-none text-white focus:outline-none resize-none overflow-y-auto outline-none transition-colors duration-200 {{ $note->bg_color ?? 'bg-brand-purple' }}"
            ...
            <div class="flex items-center justify-between shrink-0">
                <div class="flex gap-3">
                    ...
                    <div class="relative">
                        <select id="colorPicker"
                            ...
                            <option value="bg-brand-purple" {{ ($note->bg_color ?? 'bg-brand-purple') == 'bg-brand-purple' ? 'selected' : '' }}> Purple</option>
                            <option value="bg-brand-blue" {{ ($note->bg_color ?? '') == 'bg-brand-blue' ? 'selected' : '' }}> Blue</option>
                            <option value="bg-brand-green" {{ ($note->bg_color ?? '') == 'bg-brand-green' ? 'selected' : '' }}> Green</option>
                        </select>
                    </div>
                ...
                <div class="flex items-center gap-4 shrink-0">
                    <form action="{{ route('notes.destroy', $note->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this note?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
